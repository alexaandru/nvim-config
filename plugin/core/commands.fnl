...
(fn JumpToLastLocation []
  (let [buf (vim.api.nvim_get_current_buf)
        pos (vim.fn.line "'\"")]
    (if (and (> pos 0) (<= pos (vim.fn.line "$"))
             (= buf (vim.fn.winbufnr (vim.fn.winnr))))
        (vim.cmd (tostring pos)))))
...
(let [opts #(vim.tbl_extend :force {:desc $} (or $2 {}))
  ...
  (cmd :Grep "sil grep <q-args>" "Search using git grep" {:bar true :nargs "+"})
  ...
  (cmd :CdProjRoot "SetProjRoot | cd `=g:proj_root`" "Change to project root")
  (cmd :JumpToLastLocation JumpToLastLocation "Jump to last cursor position")
